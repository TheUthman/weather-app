export function getSkyGradient(type, progress) {
  if (type === "moon") {
    const horizonGlow =
      progress < 0.16 || progress > 0.84
        ? "rgba(125, 128, 180, 0.34)"
        : "rgba(69, 110, 173, 0.28)";
    return {
      background: `radial-gradient(circle at 50% 18%, rgba(207, 226, 255, 0.1), transparent 16%), radial-gradient(ellipse at 50% 108%, ${horizonGlow}, transparent 48%), linear-gradient(180deg, #01040d 0%, #06101e 25%, #0d2039 62%, #263f5e 100%)`,
    };
  }
